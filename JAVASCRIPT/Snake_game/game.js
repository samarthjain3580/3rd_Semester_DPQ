...
var highScoreSpan = document.getElementsByClassName('highscore')[0];
var score = 0;
...
function changeDirection(d) {
    ...
    switch (d) {
        case "r":
            ...
            wallCheckArray = rowEndRight;
            break;
        case "l":
            directionDeciderNum = -1;
            wallCheckArray = rowStartLeft;
            break;
        case "d":
            directionDeciderNum = tilesPerRow;
            wallCheckArray = rowEndBottom;
            break;
        case "u":
            directionDeciderNum = -tilesPerRow;
            wallCheckArray = rowStartTop;
            break;
    }
    ...
    moving = setInterval(function () {
        ...
        var currentTile = parseInt(head.dataset.tile, 10);
        var nextTileNum = currentTile + directionDeciderNum;

        // WALL COLLISION: Check if current tile is at edge and moving off-grid
        if (wallCheckArray.includes(currentTile)) {
            if (score > highScore) {
                highScore = score;
        ...
        // SELF COLLISION: Check if the next tile is part of the body
        if (body.includes(nextTileNum)) {
            if (score > highScore) {
                highScore = score;
                localStorage.setItem('highScore', highScore);
                highScoreSpan.innerHTML = "High Score: " + highScore;
...
function restartGame() {
    scoreSpan.innerHTML = +score + ". GAME OVER";
    highScoreSpan.innerHTML = "High Score: " + (localStorage.getItem('highScore') || 0);
